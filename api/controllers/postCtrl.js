...
const postCtrl = {
    ...
    createPost: async (req, res) => {
        try {
            ...
            if (images.length === 0)
                return res.status(500).json({ msg: 'Plz add your photo' })
    ...
    getOnePost: async (req, res) => {
        try {
            ...
            if (!post) return res.status(400).json({ msg: 'This posts does not exist' })
    ...
    getPostDiscover: async (req, res) => {
        try {
            ...
            // Cach 2
            //Toán tử $nin sẽ kiếm các phần tử khác với các phần tử có trong array chỉ định 
            // const features = new APIfeatures(Posts.find(
            //     { user: { $nin: [...req.user.following, req.user._id] } }), req.query
            // ).paginating()
            // const posts = await features.query.sort('-createdAt')
    ...
    savedPost: async (req, res) => {
        try {
            ...
            if (!save) res.status(400).json({ msg: 'This user does not exist' })
